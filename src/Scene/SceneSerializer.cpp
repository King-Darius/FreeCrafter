...
std::array<float, 16> transformFromJson(const QJsonArray& arr)
{
    ...
    matrix.fill(0.0f);
    const qsizetype count = std::min<qsizetype>(arr.size(), 16);
    for (qsizetype i = 0; i < count; ++i)
        matrix[static_cast<std::size_t>(i)] = static_cast<float>(arr.at(i).toDouble(matrix[static_cast<std::size_t>(i)]));
    return matrix;
}
...
SceneSerializer::Result SceneSerializer::saveToStream(const Document& document, std::ostream& stream)
{
    ...
    std::unordered_map<GeometryObject::StableId, std::size_t> stableLookup;
    const auto& objects = document.geometryKernel.getObjects();
    geometryLookup.reserve(objects.size());
    stableLookup.reserve(objects.size());
    for (std::size_t i = 0; i < objects.size(); ++i) {
        geometryLookup.emplace(objects[i].get(), i);
        stableLookup.emplace(objects[i]->getStableId(), i);
    }
    ...
    for (const auto& [stableId, name] : materials) {
        auto it = stableLookup.find(stableId);
        if (it == stableLookup.end())
            continue;
        QJsonObject matObj;
