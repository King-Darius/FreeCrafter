...
## Appendix: FreeCrafter Parity Roadmap (for AGENTS)

This section describes what FreeCrafter needs to implement (or harden) to reach parity with the target sketch-based push/pull modeling behaviour described in the main specification. Treat it as a task brief for coding agents.
...
* Qt 6 desktop shell with dockable toolbars and a measurement / value-control box.
* Hardware-accelerated OpenGL viewport.
* A half-edge geometry core with healing, triangulation, and normals.
* Core tools: Select, Line, Move, Rotate, Scale, Push/Pull, Offset, Follow-Me, Arc/Circle/Polygon, Text, Paint.
* Groups/components with make-unique flows, tag-based visibility, and an outliner.
...
1. A strict **edge → loop → face** pipeline that always keeps faces in sync with edges.
2. A **workplane-centric inference engine** with all the subtle snaps and guides.
3. **Component glue/cut semantics** plus safe selection logic around them.
4. A hardened **GPU viewport + navigation gizmo** that never lies about planarity or closure.
5. Topology-aware **selection and undo** that protects dependent faces and glued components.
...
1. **Group edges by coplanar sets.**
2. **Extract closed, non-self-intersecting loops** in each plane.
3. **Create/update faces** from these loops.
4. Nest loops to determine outer boundary vs. holes.

### 2.2 To-dos for FreeCrafter

1. **Central face solver module**
...
2. **Planarity + tolerance discipline**
...
3. **Hole vs separate face semantics**
...
4. **Healing and loop consistency**
...
1. **Introduce `WorkPlane` and `WorkPlaneManager`**
...
2. **Make drawing tools plane-aware**
...
3. **Plane-aware snapping default**
...
* Quadrant points (0°, 90°, 180°, 270°).
* Tangent points where a new line is tangent to a circle/arc.
* Nearest point on an edge or curve.
...
3. **Constraint resolution**
   ...
   * Once a snap is selected:
     ...
     * Solve the exact snap point:
       ...
       * Equal length: project along direction until distance matches reference.
       * Tangent: solve for the tangent point on circle/arc.
   * Provide a unified API, e.g.:

     * `InferenceResult { snapPoint, snapType, snapDirection, debugGuides[] }`.
...
1. User chooses a component and a host face.
2. System computes an alignment transform:

   * Component’s glue face normal → host face normal.
   * Respect a notion of “up” axis if defined.
3. Instance is glued to host plane.
4. If cutting is enabled:
...
1. **Component behaviour extension**
...
1. **Separate model vs preview vs HUD**

   * Three rendering layers:

     1. **Model**: persistent geometry from the half-edge core.
     2. **Preview**: current tool’s temporary geometry (in-progress edges, proposed faces).
     3. **HUD/Inference**: guides, texts, navigation gizmo.
   * Each layer can be updated independently without forcing full rebuilds.

2. **Unified GPU batching**
...
3. **Navigation gizmo correctness**
...
4. **Tool activation state machine**
...
1. **Global tolerances + WorkPlane abstraction**

   * [ ] Define `EPS_DIST`, `EPS_ANGLE` in core.
   * [ ] Implement `WorkPlane` + `WorkPlaneManager`.
   * [ ] Port Line/Arc/Circle tools to operate via workplanes.

2. **FaceSolver v1**

   * [ ] Implement central face solver (coplanar grouping, loop detection, face creation).
   * [ ] Route all topology-changing tools through it.
   * [ ] Implement proper outer/inner loop handling (holes).

3. **Inference v1**

   * [ ] Implement `InferenceIndex` for nearby vertices/edges/arcs.
   * [ ] Implement `InferenceSolver` for scoring snap candidates (endpoint, midpoint, axis, parallel, perpendicular, equal length, tangent, etc.).
   * [ ] Implement `InferenceOverlay` for dotted lines, arc-center cues, right-angle squares, and snap labels.
   * [ ] Integrate with Line, Arc, Circle, Move tools.

4. **Push/Pull kernel**

   * [ ] Implement `extrude_face()` for surfaces.
   * [ ] Implement context-aware boolean behaviour for solids.
   * [ ] Integrate with FaceSolver and Inference.

5. **Component behaviours**

   * [ ] Extend component definitions with glue/cut metadata.
   * [ ] Implement placement solver for aligning components to host faces.
   * [ ] Implement hole-cutting logic (project cut profile, modify host face loops).
   * [ ] Implement selection-dependency checks for glued components/hosts.

6. **Selection + undo safety**

   * [ ] Implement typed selections and helper queries (face → solid, etc.).
   * [ ] Implement a “SelectionAnalysis” preflight step for destructive operations.
   * [ ] Group low-level operations into coherent undo commands.

7. **Viewport & navigation gizmo**

   * [ ] Split rendering into model, preview, HUD/inference layers.
   * [ ] Implement stable navigation gizmo consistent with world axes.
   * [ ] Ensure tool state transitions do not break rendering or input routing.

Agents working on FreeCrafter should treat this appendix as the parity roadmap and refer back to the main interaction and modelling specification for deeper behavioural details while implementing each subsystem.
