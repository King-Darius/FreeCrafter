...
- [ ] Full regression pass across all implemented features with scripted + exploratory testing to surface functional and stability regressions before the performance push. (Blocked: build fails on `MainWindow.cpp` view menu corruption; see `docs/status/2025-10-10-phase7_5-bug-sweep.md`.)
- [ ] Source review and static analysis sweep covering core, graphics, ops, ui, and io modules to tighten ownership, lifetime, and threading assumptions. (Initial findings logged in `docs/status/2025-10-10-phase7_5-bug-sweep.md`; deeper automated pass still pending.)
- [x] Document end-to-end bug triage workflow (capture → reproduce → isolate → fix → verify) and ensure open issues are categorized by area/priority. (See `docs/process/bug_triage_workflow.md`.)
- [ ] Audit undo/redo, autosave, and crash recovery paths to confirm data integrity and user trust. (Currently failing: no `QUndoStack`, autosave manager, or recovery flow.)
- [ ] Holistic GUI/UX review against the top-down contract: verify tool wiring, visual consistency, accessibility contrast, and responsive layout behavior on all supported platforms. (Issues recorded in `docs/status/2025-10-10-phase7_5-bug-sweep.md`; theme toggle and View menu require fixes.)
- [ ] Update automated test suites and add coverage where critical workflows lack assertions (geometry ops, tool activation, document lifecycle, file I/O). (Pending until build is restored.)
